...
public class ActivityTab extends ActivityGroup {
	...
	private PromotionAdapter ppAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        new PaypalTask().execute();
		
		sliding = (SlidingDrawer)findViewById(R.id.sdUpdate);
		
		 // we instatiate the TabHost
        tabHost = (TabHost)findViewById(R.id.tabhost);
        //tabHost.setup();
        tabHost.setup(this.getLocalActivityManager());
        
        // this manages SMS operations such as sending data, text, and pdu SMS messages.
        sms = SmsManager.getDefault();
        
        TabSpec specs = tabHost.newTabSpec("tag1");
     	//specs.setContent(R.id.tab4);//set layout bang id hoac ban activity, hoactabcontenfactorry
     	intent = new Intent(this,GridViewTabActivity.class);
     
     	specs.setIndicator("Cart", getResources().getDrawable(R.drawable.pay));//dan nhan va tao bieu tuong
     	specs.setContent(intent);
     	tabHost.addTab(specs); 
     	
     	  specs = tabHost.newTabSpec("tag2");
          specs.setContent(R.id.tab1);
          specs.setIndicator("Products", getResources().getDrawable(R.drawable.shop));
          tabHost.addTab(specs);
        ...
        specs.setIndicator("Promo", getResources().getDrawable(R.drawable.back_ok));
        tabHost.addTab(specs);
        
        specs = tabHost.newTabSpec("tag4");
        specs.setContent(R.id.tab4);
        specs.setIndicator("Cart", getResources().getDrawable(R.drawable.pay));
        tabHost.addTab(specs);
        
        specs = tabHost.newTabSpec("tag5");
        specs.setContent(R.id.tab3);
        specs.setIndicator("SendTo", getResources().getDrawable(R.drawable.email));
        tabHost.addTab(specs);
        ...
        pAdapter = new ProductsAdapter(pName);
        lvProducts = (ListView)findViewById(R.id.lvProducts);
        lvProducts.setCacheColorHint(Color.TRANSPARENT);
        lvProducts.setAdapter(pAdapter);
        
        // this is for list of what we want to buy
        cAdapter = new CartAdapter(cName);
        lvCart = (ListView)findViewById(R.id.lvCart);
        lvCart.setCacheColorHint(Color.TRANSPARENT);
        lvCart.setAdapter(cAdapter);
        
        ppAdapter = new PromotionAdapter(ppName);
        lvPromotions = (ListView)findViewById(R.id.lvPromo);
        lvPromotions.setCacheColorHint(Color.TRANSPARENT);
        lvPromotions.setAdapter(ppAdapter);
        
        etEmail = (EditText)findViewById(R.id.etEmail);
        btnSend = (Button)findViewById(R.id.btnSend);
        btnHandle = (Button)findViewById(R.id.handle);
       ...
       // btnBuy=(Button)findViewById(R.id.btnBuy);
        ...
        tvValueTotalQty = (TextView)findViewById(R.id.tvValueTotalQty);
        tvValueTotalUSD = (TextView)findViewById(R.id.tvValueTotalUSD);
        
        utils = new Utils(ActivityTab.this);
        db = new EcommerceDatabase(ActivityTab.this);
        
        rlPay = (RelativeLayout)findViewById(R.id.rlPay);
        rlLoading = (RelativeLayout)findViewById(R.id.rlLoading);
        
        imgLoading = (ImageView)findViewById(R.id.imgLoading);

        scaleIn = AnimationUtils.loadAnimation(getApplicationContext(), R.anim.scale);
        translateInUp = AnimationUtils.loadAnimation(getApplicationContext(), R.anim.translate_in_top);
        translateInDown = AnimationUtils.loadAnimation(getApplicationContext(), R.anim.translate_in_down);
	 ...
	 public void switchTab(int tab,String searchName){
		 SharedPreferences sharedPref =getPreferences(Context.MODE_WORLD_WRITEABLE);
		   String loaiSp = sharedPref.getString("loaiSp", "Graphics");
	        catSearch = searchName;
	...
	protected void onStart() {
		...
		lvPromotions.setOnScrollListener(new OnScrollListener() {
		    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
		        //Check if the last view is visible
		        if (firstVisibleItem + visibleItemCount >= totalItemCount) {
		        	if(!Utils.IS_ENDED_PROMO_LIST && !IS_PROMOTIONS_TASK){
		        		fillPromotionList();
		        	}
		        }   
		    }
		...
		btnRegister.setOnClickListener(new OnClickListener(){
			public void onClick(View view){
				dialog = new MyAlertDialog(ActivityTab.this);
...
//			        }
	 ...
	 */
	private void fillProductListFromTable(){
		// we check if there is an internet connection and if the address it is ok
		if(CustomHttpClient.isAddressOk(Utils.LINK_PRODUCTS)){
			// this represent the starting point from MYSQL LIMIT, so if it 10 will return from the 10th row
			// we need to know how many rows we downloaded before, and stored on the sqlite db
			long from = db.size(EcommerceDatabase.TABLE_PRODUCTS);
...
//        	int mysqlSize = getMysqlTableSize(EcommerceDatabase.TABLE_PRODUCTS);
			...
			if(from == 0){
				...
				// if is not 0, first will parse all the rows to arraylists from sqlite stored on the phone
				// this is for the time when we want to update async the data, so first we load all the info stored in sqlite and then
				// we parse it from the mysql by taking from the total of the rows stored in sqlite and more
				Cursor cursor = db.getFirstRows();
				if(cursor != null && cursor.getCount()!=0){
					cursor.moveToFirst();
					while(!cursor.isAfterLast()){
				...
				// after we took all the rows from our sqlite db, then we request the next 10 rows from mysql if there are
				// we add the post variables and values for the request
	        ...
	        try {  
	        	String response;
	        	// Enter the remote php link
	        	response = CustomHttpClient.executeHttpPost(Utils.LINK_PRODUCTS, postParameters).toString(); 
	        	
	        	// we convert the response into json array
	        	jarray = utils.getJarrayFromString(response);
	        	...
	        	if(jarray.length()>0){
		        	for(int i=0; i<jarray.length(); i++){
		        		...
		        		String iconPath = Utils.LINK_IMAGES + row.getString("pIcon");
		        		utils.downloadImage(iconPath, row.getString("pIcon"));
			...
			if(db.size(EcommerceDatabase.TABLE_PRODUCTS) > 0){
				Cursor cursor = db.getAllTheRows(EcommerceDatabase.TABLE_PRODUCTS);
				cursor.moveToFirst();
				while(!cursor.isAfterLast()){
					pid.add(cursor.getInt(cursor.getColumnIndex(EcommerceDatabase.KEY_PID)));
	        		pName.add(cursor.getString(cursor.getColumnIndex(EcommerceDatabase.KEY_PNAME)));
	...
	/**
	 * This method will fill the lvProducts, first will check to see if you are connected to the internet and if the address it is working, if it true 
	 * than will start to parse from mysql only the products that are not on a promotion(WHERE pPromotion=false) database through json, an each row parsed from 
	 * mysql, it will create a new row on sqlite db, if it is false then will try to parse from sqlite db. Here we use products.php script to access data.
	 */
...
//			long sqliteSize = db.size(EcommerceDatabase.TABLE_PRODUCTS);
	...
	private void fillPromotionList(){
		// we check if there is an internet connection and if the address it is ok
		if(utils.isInternet() && CustomHttpClient.isAddressOk(Utils.LINK_PROMOTIONS)){
			// this represent the starting point from MYSQL LIMIT, so if it 10 will return from the 10th row
			// we need to know how many rows we downloaded before
			final int from = ppid.size();
			// this represent how many rows will return from the starting point from MYSQL LIMIT
			final int nr = 5;
//			long sqliteSize = db.size(EcommerceDatabase.TABLE_PRODUCTS);
			...
			// if this is 0 then we will take the first 10 rows from mysql
				...
				task = new RequestTask("promotions", postParameters);
	        	task.setOnTaskCompleted(new OnTaskCompletedListener(){
	        		public void onTaskStarted() {
	        			IS_PROMOTIONS_TASK = true;
					}
					public void onTaskCompleted(String result) {
						try { 
				        	// we convert the response into json array
							if(result!="") {
					        	...
					        	if(from <= mysqlSize){
						        	...
						        	if(jarray.length()>0){
							        	...
							        	for(int i=1; i<rows; i++){
							        		JSONObject row = jarray.getJSONObject(i);
							        		ppid.add(row.getInt("pid"));
							        		ppName.add(row.getString("pName"));
							        		ppTitle.add(row.getString("pTitle"));
							        		ppPrice.add(row.getString("pPrice") + "$");
							        		ppDescription.add(row.getString("pDescription"));
							        		ppCategory.add(row.getString("pCategory"));
							        		ppIcon.add(Utils.PATH + row.getString("pIcon"));
							        		ppDiscount.add(row.getInt("pDiscount"));
							        		list.add(row.getString("pIcon"));
							        		// we check if this id already exists in the db, if it doesn't exists we create new one
							        		if(!db.hasIDProducts(row.getInt("pid"))) db.createRowOnProducts(row.getInt("pid"), row.getString("pName"), row.getString("pTitle"), row.getString("pPrice"), row.getString("pDescription"), row.getString("pCategory"), Utils.PATH + row.getString("pIcon"), row.getString("pPromotion"), row.getInt("pDiscount"), row.getString("pDate"));
							        	}
							        	new DownloadImages(list, ppAdapter).execute();
						        	}
					        	}
					        	postParameters.removeAll(postParameters);
							}else{
			...
			// if we are not connected on internet or somehow the link would not work, then we will take the rows stored in sqlite db
			if(db.size(EcommerceDatabase.TABLE_PRODUCTS) > 0){
				Cursor cursor = db.getPromotionalRows(EcommerceDatabase.TABLE_PRODUCTS);
				cursor.moveToFirst();
				while(!cursor.isAfterLast()){
					ppid.add(cursor.getInt(cursor.getColumnIndex(EcommerceDatabase.KEY_PID)));
	        		ppName.add(cursor.getString(cursor.getColumnIndex(EcommerceDatabase.KEY_PNAME)));
	        		ppTitle.add(cursor.getString(cursor.getColumnIndex(EcommerceDatabase.KEY_PTITLE)));
	        		ppPrice.add(cursor.getString(cursor.getColumnIndex(EcommerceDatabase.KEY_PPRICE)) + "$");
	        		ppDescription.add(cursor.getString(cursor.getColumnIndex(EcommerceDatabase.KEY_PDESCRIPTION)));
	        		ppCategory.add(cursor.getString(cursor.getColumnIndex(EcommerceDatabase.KEY_PCATEGORY)));
	        		ppIcon.add(cursor.getString(cursor.getColumnIndex(EcommerceDatabase.KEY_PICON)));
	        		ppDiscount.add(cursor.getInt(cursor.getColumnIndex(EcommerceDatabase.KEY_PDISCOUNT)));
	        		cursor.moveToNext();
				}
    ...
    private class PromotionAdapter extends BaseAdapter{
    	...
    	public PromotionAdapter(ArrayList<String> ll){
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			Bitmap bmp = BitmapFactory.decodeFile(ppIcon.get(position));
			holder.imgIcon.setImageBitmap(bmp);
			holder.tvName.setText(ppName.get(position));
			holder.tvTitle.setText(ppTitle.get(position));
			holder.tvPrice.setText(ppPrice.get(position));
		...
		public View getView(int position,   View convertView, ViewGroup parent) {
	       ...
	       holder.btnBuy.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					popup.setAnimationStyle(R.style.animation_popup);
		           // popup.showAtLocation(lvProducts,Gravity.NO_GRAVITY,display.getWidth()/2, display.getHeight()/2);
	        		//arg1.getTop() return the top of this view, in pixels. 
	        		popup.showAtLocation(lvProducts, Gravity.NO_GRAVITY, b.getWidth()/2-Resizable.getDrawableWidth(getResources(), R.drawable.popup_menu)/2, b.getTop()+b.getHeight()/2);
		            
	        		setPopupListeners(viewInflated, a, pid);
			//aa
    ...
    private class CartAdapter extends BaseAdapter{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			Bitmap bmp = BitmapFactory.decodeFile(cIcon.get(position));
	...
	class PaypalTask extends AsyncTask<Void, Void, Void> {
		public Void doInBackground(Void... arg0) {
			 ...
			 * server: (Required) Sets the PayPal server to Live, Sandbox, or None.
		...
		protected void onPostExecute(Void result) {
			// TODO Auto-generated method stub
			super.onPostExecute(result);
			 // You must get the Pay with PayPal payment button from the Mobile Payments Library. 
	        // Use this method, which returns a CheckoutButton (a subclass of LinearLayout), which you can place in your application.
	        ...
	        btnPaypal.setOnClickListener(new OnClickListener(){
				public void onClick(View view) {
					...
					// we check to see if we have some products in shopping cart
					if(!cid.isEmpty()){
						PayPalPayment payment = new PayPalPayment();
						payment.setSubtotal(new BigDecimal(TOTAL_USD));
						...
						// in order to be able to click again on the paypal button, we need to update it
						btnPaypal.updateButton();
					}
				}	
			});
